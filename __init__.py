...
class ProfilerTool(object):
    ...
    def func_calls(self):
        ...
        for func in self.stats.sort_stats('cumulative').fcn_list:
    ...
    def data(self):
        total_time = self.total_time
        func_calls = self.func_calls[:20]
        fields = ['ncalls', 'tottime', 'percall', 'cumtime', 'percall_cum', 'filename']
        profile_data = {'total_time': total_time, 'func_calls': []}
        for func in func_calls:
            d = {}
            for field in fields:
                d[field] = func.get(field)
            profile_data['func_calls'].append(d)
        return profile_data
    ...
    def after_request(cls, response):
        ...
        elif response.status_code == 200 and response.headers['content-type'].startswith('application/json'):
            data = response.data.decode(response.charset)
            data = json.loads(data)
            profile_data = request._profiler_tool.data
            if isinstance(data, dict):
                data['profile_data'] = profile_data
            else:
                data.append(profile_data)
            data = json.dumps(data, indent=2)
            response.data = data.encode(response.charset)
        return response
